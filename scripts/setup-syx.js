...
async function getEvents(tx) {
  ...
  return receipt.logs.reduce((parsedEvents, log) => {
    try {
      parsedEvents.push(syxBuilder.interface.parseLog(log))
    } catch (e) {}
    return parsedEvents
  }, [])
}
...
const setup = async () => {
  ...
  syxBuilder = await ethers.getContractAt(
    "SyxPrizePoolBuilder",
  ...
  let tx = await syxBuilder.createSyxSingleWinner(syxPrizePoolConfig, syxSingleWinnerConfig, decimals)
  ...
  const prizeStrategy = await ethers.getContractAt("SyxSingleWinner", await prizePool.prizeStrategy(), wallet)
  ...
  //if(isTestEnvironment){
  let createTx = await syxBuilder.createSponsor()
