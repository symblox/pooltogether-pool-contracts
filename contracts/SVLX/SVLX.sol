...
contract SVLX is ReentrancyGuard {
  ...
  event SetValidatorSetContract(address oldValidatorSetContract, address newValidatorSetContract);
  ...
  function deposit() public payable nonReentrant {
    ...
    if (msg.value >= minStake || auRa.stakeAmount(currentPool, address(this)) > 0) {
      IStakingAuRa(stakingAuRa).stake{ value: msg.value }(currentPool, msg.value);
      emit Deposit(msg.sender, msg.value);
    } else if (currentBalance >= minStake) {
      IStakingAuRa(stakingAuRa).stake{ value: currentBalance }(currentPool, currentBalance);
  ...
  function withdraw(uint256 wad) public nonReentrant returns (uint256) {
    ...
    for (uint256 i = 0; i < pools.length(); ++i) {
      ...
      uint256 needToWithdraw = wad > currentBalance ? wad.sub(currentBalance) : 0;
      if (needToWithdraw == 0) {
        break;
      }
      if (_isWithdrawAllowed(pools.at(i))) {
        ...
        // 锁仓数量
        uint256 stakeAmount = auRa.stakeAmount(pools.at(i), address(this));
        ...
        if (maxWithdrawal > 0) {
          ...
          if (threshold > needToWithdraw) {
            auRa.withdraw(pools.at(i), needToWithdraw.min(maxWithdrawal));
            emit PoolWithdraw(pools.at(i), needToWithdraw.min(maxWithdrawal));
          } else {
    ...
    for (uint256 i = 0; i < pools.length(); ++i) {
      ...
      if (_isWithdrawAllowed(pools.at(i))) {
        uint256 maxOrderWithdrawal = auRa.maxWithdrawOrderAllowed(pools.at(i), address(this));
        address _miningAddress = validatorSet.miningByStakingAddress(pools.at(i));
        if (validatorSet.isValidatorOrPending(_miningAddress) && maxOrderWithdrawal > 0) {
    ...
    )
  {
    ...
    for (uint256 i = 0; i < length; ++i) {
      uint256 claimAmount = auRa.orderedWithdrawAmount(pools.at(i), address(this));
      _pools[i] = pools.at(i);
      _amount[i] = claimAmount;

      // 如果是在当前 epoch order 的数量，在当前 epoch 结束后的下一个区块可以取出
      // 如果直接可以取出，返回0
      if (stakingEpoch == auRa.orderWithdrawEpoch(pools.at(i), address(this)) && claimAmount > 0) {
  ...
  function claimableOrderedAmount(address poolAddress) internal view returns (uint256) {
    IStakingAuRa auRa = IStakingAuRa(stakingAuRa);
    uint256 stakingEpoch = auRa.stakingEpoch();
    uint256 claimAmount = auRa.orderedWithdrawAmount(poolAddress, address(this));
    if (stakingEpoch > auRa.orderWithdrawEpoch(poolAddress, address(this)) && claimAmount > 0) {
      return claimAmount;
    }
    return 0;
  }
  ...
  function claimOrderedWithdraw(address poolAddress) internal returns (uint256) {
    IStakingAuRa auRa = IStakingAuRa(stakingAuRa);
    uint256 stakingEpoch = auRa.stakingEpoch();
    uint256 claimAmount = auRa.orderedWithdrawAmount(poolAddress, address(this));
    if (stakingEpoch > auRa.orderWithdrawEpoch(poolAddress, address(this)) && claimAmount > 0) {
  ...
  /// @notice 是否可以取款
  function _isWithdrawAllowed(address poolAddress) internal view returns (bool) {
    address _miningAddress = IValidatorSetAuRa(validatorSetContract).miningByStakingAddress(poolAddress);
    if (IValidatorSetAuRa(validatorSetContract).areDelegatorsBanned(_miningAddress)) {
  ...
  function getPoolsStaked() public view returns (address[] memory pool, uint256[] memory stake) {
    IStakingAuRa auRa = IStakingAuRa(stakingAuRa);
    uint256 length = pools.length();
  ...
  function setValidatorSetContract(address _validatorSetContract) public onlyAdmin {
    address oldValidatorSetContract = validatorSetContract;
    validatorSetContract = _validatorSetContract;

    emit SetValidatorSetContract(oldValidatorSetContract, validatorSetContract);
  }
  ...
  function claimReward(uint256[] memory _stakingEpochs, address _poolStakingAddress) public {
    IStakingAuRa auRa = IStakingAuRa(stakingAuRa);
